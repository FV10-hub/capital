...
public class CobRecibosController {
    ...
    public void setCobReciboCabeceraSelected(CobReciboCabecera cobReciboCabeceraSelected) {
        if (!Objects.isNull(cobReciboCabeceraSelected)) {
            ...
            this.cobCobrosValoresSelected.setMontoValor(cobReciboCabecera.getMontoTotalRecibo());
    ...
    public CobCobrosValores getCobCobrosValoresSelected() {
        if (Objects.isNull(cobCobrosValoresSelected)) {
            ...
            System.out.println("se reinicio los valores");
        }
    ...
    public LazyDataModel<CobSaldo> getLazyModelSaldos() {
        if (Objects.isNull(lazyModelSaldos)) {
            ...
            Long clienteId = (cobReciboCabecera != null && cobReciboCabecera.getCobCliente() != null)
                    ? cobReciboCabecera.getCobCliente().getId()
                    : null;
            String tipo = StringUtils.isNotBlank(tipoSaldoAFiltrar) ? tipoSaldoAFiltrar : null;

            List<CobSaldo> lista = cobSaldoServiceImpl.buscarSaldosFiltrados(empresaId, clienteId, tipo);
            lazyModelSaldos = new GenericLazyDataModel<>(lista);
