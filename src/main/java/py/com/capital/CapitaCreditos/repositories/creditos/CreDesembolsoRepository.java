...
public interface CreDesembolsoRepository extends JpaRepository<CreDesembolsoCabecera, Long> {
    ...
    @Query("""
                UPDATE CreDesembolsoCabecera s
                   SET s.indContratoImpreso = true
                 WHERE s.bsEmpresa.id = :empresaId
                   AND s.id = :desembolsoId
            """)
    int marcarContratoImpresa(@Param("empresaId") Long empresaId,
                              @Param("desembolsoId") Long desembolsoId);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("""
                UPDATE CreDesembolsoCabecera s
                   SET s.indPagareImpreso = true
                 WHERE s.bsEmpresa.id = :empresaId
                   AND s.id = :desembolsoId
            """)
    int marcarPagareImpresa(@Param("empresaId") Long empresaId,
                            @Param("desembolsoId") Long desembolsoId);
